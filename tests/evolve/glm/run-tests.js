...
function generateFindings() {
  ...
  const skippedCount = tests.filter((t) => t.status === "SKIP").length;
  if (skippedCount > 0) {
    findings += `- **Skipped (NOT run, no coverage)**: ${skippedCount}\n`;
  }

  // "ALL TESTS PASSED" alongside a non-zero skip count is the report claiming
  // coverage it does not have. ATTACK-6b (lease expiry and re-acquisition) is
  // hard-coded to SKIP on every run, so this line used to assert that a property
  // nobody checked held.
  if (failed === 0 && errors === 0 && skippedCount === 0) {
    findings += `\n## ✅ ALL TESTS PASSED - No defects found\n\n`;
  } else if (failed === 0 && errors === 0) {
    findings += `\n## ⚠️ NO FAILURES, BUT ${skippedCount} TEST(S) DID NOT RUN\n\n` +
      `Every test that executed passed. The skipped test(s) below were not run, so this\n` +
      `report says nothing about the properties they cover.\n\n` +
      tests.filter((t) => t.status === "SKIP").map((t) => `- \`${t.name}\` — ${t.reason || "no reason given"}`).join("\n") +
      `\n\n`;
  ...
  // See the note in tests/attacks/deepseek/run-tests.js: a wall-clock stamp in a
  // COMMITTED report dirties a tracked file on every run for no signal.
  findings += `- **Regenerated by**: node tests/evolve/glm/run-tests.js\n`;
...
function main() {
  ...
  // Do not claim a total that includes tests which never ran.
  const skippedTotal = tests.filter(t => t.status === "SKIP").length;
  const ranTotal = tests.length - skippedTotal;
  if (skippedTotal > 0) {
    console.log(`\n⚠️  NO FAILURES: ${ranTotal}/${tests.length} tests ran and passed; ` +
      `${skippedTotal} did NOT run and are unproven`);
  } else {
