...
function nc_coverageGap() {
  ...
  // This control executes nothing: it scrapes an 800-byte window of a DIFFERENT
  // suite's source for a literal phrase. When toctou's UNAVAILABLE message was
  // reworded (it now reads "not proven in this single-process harness ... deferred to
  // a multi-process platform-probe cell"), the phrase vanished and this control
  // silently flipped NC_PASS -> FINDING, reporting a dishonesty defect that does not
  // exist because a string moved. Match on what the message must MEAN, and make a
  // broken anchor say so instead of scoring the suite.
  if (unavFn === -1) {
    finding(nameUnav, "INCONCLUSIVE (harness): function race_trueParallelRename was not " +
      "found in tests/attacks/toctou/run-tests.js, so this control scanned nothing -- it " +
      "was renamed or moved and this check needs re-anchoring, which is a defect in THIS " +
      "control, not in toctou");
    return;
  }
  const admitsHarnessLimit =
    /inherently unprovable|not proven in this single-process harness|single-process|single-thread/i.test(unavBody);
  const declaresUnavailable = /UNAVAILABLE|unavailable/.test(unavBody);
  if (admitsHarnessLimit && declaresUnavailable) {
...
// This report is COMMITTED, so a wall-clock stamp in it makes every test run
// dirty a tracked file. That churn is why `git add` after a run sweeps in
// unrelated files and why a real edit is hard to tell from test noise. The
// report's content still diffs when a verdict genuinely changes -- which is
// the signal worth keeping -- but the timestamp carried no signal at all.
...
Generated: regenerated by: node tests/attacks/deepseek/run-tests.js
