...
**Regenerated by:** node tests/diagnostics/glm/run-tests.js
**Security Tier:** I4 - 'GraphSmith sends nothing upstream, ever'
...
function main() {
  ...
  try {
    ...
    // teardown() is wired as `finally { teardown(); }` around this block, but
    // process.exit() terminates the process immediately without unwinding, so the
    // finally never ran on either of these two paths -- cleanup only fired on an
    // uncaught throw, the path nobody takes, and every normal run left
    // tests/diagnostics/glm/temp-projects/ behind in the repo tree.
    teardown();
