...
Lease + heartbeat semantics from [KnoSky: scripts/scaffold.js:64-113], with: (a) lock content = `{ pid, owner_token: random128 }`; renew/release/steal verify `owner_token` (compare-before-write) — pid-reuse cannot renew another owner's lease; (b) promotion lease/heartbeat bounds are frozen in the release manifest; `GRAPHSMITH_LEASE_MS`/`GRAPHSMITH_HEARTBEAT_MS` are honored ONLY when `GRAPHSMITH_TEST_MODE=1` (a mode the sentinel reports and CI forbids in release jobs); (c) recovery acquires the lease BEFORE reading the journal; (d) a lock whose mtime is more than `LOCK_CLOCK_SKEW_TOLERANCE_MS` AHEAD of this host's clock has an unusable age and MUST NOT be read as freshness — the decision falls to observed owner liveness alone, and a refusal in that state MUST name clock skew rather than report a negative age.

**Amendment 2026-08-01 — `proc_start_hint` removed from the lock record.** It was written by every acquire and read by nothing. Clause (a) previously listed it, which is exactly the harm: a reader of this contract would infer a process-identity defence against pid reuse that did not exist anywhere in the implementation. Pid reuse is already bounded by (a) plus the steal rule — a reused pid can cause a REFUSAL, never a false steal, and that refusal self-heals once the lock goes unrenewed. It is no longer written.

The field remains **accepted but ignored** in `schemas/state-store.schema.json` (removed from `required`, retained in `properties`) and this is normative, not leftover tidying: the lock schema is `additionalProperties: false`, so removing the property outright would make every lock file written by a prior version fail validation as `CORRUPT_STATE` — a state the acquire path cannot recover from — turning an upgrade into an outage. Any future removal of the property MUST be preceded by a migration that can rewrite or condemn a legacy lock without taking the lock.
