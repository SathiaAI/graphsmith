...
/* Round-N fix (frontier-panel finding #10): connectionId here can originate from the
 * remote agent side of the transport (it is not generated by this gateway), so building
 * a filesystem path directly from it is unsafe the same way recovery.js's own header
 * (see its SAFE_ID/assertSafeId) already documents for its own path-construction points
 * -- an unvalidated "../../etc/whatever" (or similar) connectionId could otherwise address
 * a file outside this quarantine directory. Kept local to this file rather than importing
 * recovery.js's private assertSafeId (not exported, and this call site's failure mode is
 * different: quarantining is already best-effort, so an unsafe id degrades to a fixed
 * placeholder filename rather than aborting the whole quarantine attempt). The raw,
 * unsanitized connectionId is still recorded in the JSON body below for operator
 * traceability even when the filename itself had to be substituted. */
const QUARANTINE_SAFE_ID = /^[A-Za-z0-9._-]+$/;
function quarantineFilenameId(connectionId) {
  if (typeof connectionId === "string" && connectionId !== "." && connectionId !== ".." && QUARANTINE_SAFE_ID.test(connectionId)) {
    return connectionId;
  }
  return "unsafe-connection-id";
}
...
function quarantineSealedBundle(stateDir, connectionId, sealed, cause) {
  try {
    ...
    /* Round-N fix (frontier-panel finding #6): mkdirSync's own `mode` option is IGNORED
     * for a directory that already existed before this call (recursive:true does not
     * revisit or retighten an existing directory's mode) -- see chain.js#appendSession's
     * identical fix and doc comment for the same underlying Node behavior. Best effort:
     * a failure here (e.g. this process does not own the directory) must not block the
     * quarantine write itself. */
    try {
      fs.chmodSync(dir, stateStore.DEFAULT_DIR_MODE);
    } catch (error) {
      /* best effort -- see doc comment above */
    }
    /* Round-N fix (frontier-panel finding #10): Date.now() gives millisecond resolution,
     * so two quarantine writes for the same connectionId within the same millisecond
     * (a realistic case: a poisoned connection can be closed and immediately reopened/
     * re-quarantined by a retrying agent) collide on the same filename -- flag:"wx" then
     * makes the SECOND write throw and silently discard that sealed bundle via this
     * function's own best-effort catch below, exactly the kind of silent data loss
     * quarantining exists to prevent. crypto.randomBytes is cheap and collision-safe
     * enough here (this is a diagnostic artifact, not a security token). */
    const target = path.join(dir, `${quarantineFilenameId(connectionId)}-${crypto.randomBytes(8).toString("hex")}.json`);
    /* Round-N fix (frontier-panel finding #10): the raw writeFileSync/flag:"wx" here never
     * fsynced the file or its parent directory, so a quarantine write that appeared to
     * succeed could still be lost on a crash shortly after -- for a directory (`dir`)
     * already guaranteed to exist by the mkdirSync above, stateStore.atomicOverwriteFile
     * is the same already-reviewed temp-file+fsync+rename+directory-fsync primitive every
     * other durable state write in this codebase uses (see state-store.js's own header);
     * reused here rather than hand-rolling new fsync logic. connection_id is included in
     * the body (not just derived from the filename) so an operator can recover the real,
     * original connectionId even when quarantineFilenameId above had to substitute a
     * placeholder for an unsafe one. */
    const payload = JSON.stringify({ sealed, connection_id: connectionId, quarantined_at: new Date().toISOString(), reason: cause && cause.message }, null, 2);
    stateStore.atomicOverwriteFile(target, payload, dir, { mode: stateStore.DEFAULT_FILE_MODE });
    return target;
...
class GatewayProxy {
    ...
    if (method === "tools/call" || isModelCallMethod(method)) {
      ...
      if (method === "tools/call") {
        ...
        if (intentDecision.kind === "replay") {
           ...
           * "make the gap observable" proposal over the majority's fail-open-and-document
           * position): both anomalies below now carry call_seq (this replayed step's own
           * identity) so a normally-sealed bundle's anomaly list says exactly WHICH step's
           * WAL record is in question, not just that some unspecified append failed --
           * previously indistinguishable from any other WAL failure in the same session.
           * This does not change the deliberate fail-open durability posture documented
           * above (append failure still poisons and still returns the cached result) --
           * only what gets recorded about it. See gateway.js#recoverCrashedSessions for
           * the companion half: an explicit INCOMPLETE marker when a crash leaves this
           * exact replayed step's CALL_START durably written but its CALL_RESULT missing. */
          if (s.walPoisoned) {
            session.recordAnomaly(s, { kind: "GATEWAY_RECOVERY_WAL_APPEND_SKIPPED_POISONED", tool: toolName, call_seq: replayCallSeq, intent_key: intentKey, detail: s.walPoisoned.reason });
          } else {
            try {
              ...
              poisonWalOnFailure(s, walError, this.now);
              session.recordAnomaly(s, { kind: "GATEWAY_RECOVERY_WAL_APPEND_FAILED", tool: toolName, call_seq: replayCallSeq, intent_key: intentKey, detail: `replayed step: ${walError.message}` });
            }
          }
        ...
        if (intentDecision.kind === "block") {
          ...
          if (s.walPoisoned) {
            this.safeLog(JSON.stringify({ event: "gateway_wal_append_skipped_poisoned", connection_id: connectionId, tool: toolName, type: "ANOMALY", detail: s.walPoisoned.reason }));
          } else {
            try {
              ...
              poisonWalOnFailure(s, walError, this.now);
              this.safeLog(JSON.stringify({ event: "gateway_wal_append_failed", connection_id: connectionId, tool: toolName, type: "ANOMALY", detail: walError.message }));
            }
          }
          this.safeLog(JSON.stringify({ event: "gateway_call_completed", connection_id: connectionId, step: null, tool: toolName, server: serverName, status: "blocked", duration_ms: 0 }));
          if (isNotification) return null;
          return {
            jsonrpc: "2.0",
            id,
      ...
      if (method === "tools/call") {
        ...
        try {
          ...
          this.safeLog(JSON.stringify({ event: "gateway_intent_update_skipped", connection_id: connectionId, tool: toolName, intent_key: intentKey, reason: "intent no longer exists (concurrently removed by close or an operator resolution)" }));
        }
      }
  ...
  async closeConnection(connectionId, reason) {
    ...
    /* Round-N fix (frontier-panel finding #1): listIntentsForConnection/updateIntent are
     * disk I/O (recovery.js) and a failure partway through this fencing loop (e.g. an I/O
     * error reading/writing an intent file -- exactly the fixture agent-transport.js's own
     * stdio-disconnect catch and gateway.js's own shutdown-loop catch already name and
     * handle) previously threw out of closeConnection BEFORE the two deletes below ran,
     * leaving a ghost entry in this.sessions/this.agentInitialized forever --
     * openConnection's own admission checks and every other lookup key off connectionId,
     * so a ghost session could never be closed or reopened again through the normal
     * lifecycle, even though every existing caller already treats this exact failure as
     * one it catches and continues past. Deliberately try/FINALLY, not try/catch: every
     * caller of closeConnection already has its own established catch (and, after this
     * same round's agent-transport.js fix, log) convention for a failure here -- this fix
     * only guarantees the cleanup always runs, without swallowing the exception itself
     * (callers still see and handle it exactly as before). One observable side effect: a
     * connection whose intent-fencing failed during an ordinary (non-shutdown) disconnect
     * is now actually removed from this.sessions immediately, so it can no longer be
     * redundantly re-attempted (and re-logged) by gateway.js's own shutdown loop later --
     * see agent-transport.js's own updated doc comment for the corresponding logging fix
     * this made necessary. */
    try {
