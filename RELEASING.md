...
### 8.8 GSA release evidence package

Every release publishes a **release evidence package** — the artifacts a reviewer or internal auditor needs to check "what ran, under what controls, tamper-evidently, and how honestly it is scoped." The package lists:

- Signed GSA bundle(s) for the reviewed workflow(s) + the reference verifier's PASS report
- The capability-profile attestation (unavailable profiles render unavailable, never green)
- [`KNOWN-LIMITATIONS.md`](KNOWN-LIMITATIONS.md) — the consolidated honesty boundary (what GSA verifies / does not verify; replay, external side-effect, generated-skill, per-skill capability, and signer-trust limitations)
- [`docs/examples/offline-verify.md`](docs/examples/offline-verify.md) — the worked offline-verifier transcript (real captured output, network denied)
- `THREAT_MODEL.md` (residual risks) + the standing adversarial-review battery results with CI-run links
- The compliance-evidence crosswalk (each obligation → the control that produces evidence; manual-only obligations marked, never covered)

Checklist:

- [ ] `KNOWN-LIMITATIONS.md` is present at repo root, linked from the README, and passes docs-lint + hygiene-scan
- [ ] `KNOWN-LIMITATIONS.md` consolidates (does not invent) the caveats from SECURITY.md, THREAT_MODEL.md, the compliance crosswalk, and the README, in required honest-language forms
- [ ] The worked offline-verifier transcript (`docs/examples/offline-verify.md`) is committed with **real captured output** (reproducible via `node scripts/gsa-offline-demo.js`), showing the ordered §9 checks; any unavailable step (e.g. replay on a model-dependent run) is shown as `UNAVAILABLE`, never green
- [ ] The copy in both deliverables was written and reviewed by **different model families** (writer ≠ reviewer)
- [ ] The release notes link the release evidence package

## Noise-floor reporting

Release notes must include the held-out table, noise floor, and flat-is-flat reporting from the evaluator-stability memo (`docs/EVALUATOR-STABILITY.md`, generated by `scripts/shadow.js`; its `regression_sensitivity_scope` field discloses that detection is Tier 1/2 only).
